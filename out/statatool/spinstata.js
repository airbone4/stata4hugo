    ...
    if (!text) {
        ...
        if (!keep) {
            process.on('exit', () => {
                fs.unlink(rfile, err => {
                    if (err)
                        console.error(`Error deleting ${rfile}:`, err);
                });
            });
        }
        return spin_lang(rfile, Object.assign({ precious: keep, comment: ['^/[*][*]', '^.*[*]/[*] *$'], language: 'stata' }, options));
    }
    else {
        return spin_lang(lines, Object.assign({ precious: keep, comment: ['^/[*][*]', '^.*[*]/[*] *$'], language: 'stata', text: true }, options));
    }
}
/**
...
async function spin_lang(source, options = {}) {
    const { knit = true, report = true, text = null, format = 'Rmd', doc = '^#+\'[ ]?', inline = '^[{][{](.+)[}][}][ ]*$', comment = ['^[# ]*/[*]', '^.*[*]/ *$'], precious = !knit && !text, language = 'R' } = options;
    let lines;
    if (!text) {
...
//# sourceMappingURL=spinstata.js.map